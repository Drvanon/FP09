...
\begin{figure}[ht]
    ...
    \begin{circuitikz}[scale = .6, transform shape]
        ...
        \draw (2, 0)    to[vR, l=$g_{x, j}$, *-] (2, -2)
                        to[battery1, l=$E_{x, j}$] (2, -3) node[ground] {} (2, -4);
        \draw (4, 0)    to[vR, l=$g_{i, j}$, *-] (4, -2)
                        to[battery1, l=$E_{g, j}$] (4, -3) node[ground] {} (4, -4);
...
devices since the average frequency of random spikes can be controlled (activity) 
and besides the frequency stochastic properties of the noise can be determined  
(by choosing the CV).
